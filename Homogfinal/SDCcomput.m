...
w1begst=findobj('Tag','wbeg');
w1beg=eval(get(w1begst,'String'));
w1endst=findobj('Tag','wend');
w1end=eval(get(w1endst,'String'));
w2ndst=findobj('Tag','w2nd');
w2nd=eval(get(w2ndst,'String'));
%w2endst=findobj('Tag','w2end');
w2end=w2nd;
%w2end=eval(get(w2endst,'String'));
w2spst=findobj('Tag','w2sp');
wspacing=eval(get(w2spst,'String'));

w2sp=wspacing;
w3st=findobj('Tag','w3rd');
...
Afwphasest=findobj('Tag','Afwphase');
Afwphase=eval(get(Afwphasest,'String'));
Abwphasest=findobj('Tag','Abwphase');
Abwphase=eval(get(Abwphasest,'String'));
...
tspacingst=findobj('Tag','tspacing');
tspacing=eval(get(tspacingst,'String'));
trangest=findobj('Tag','trange');
trange=eval(get(trangest,'String'));
tvec=(-trange:tspacing:trange);
...
w1stcoeffst=findobj('Tag','w1stcoeff');
w1stcoeffarr=get(w1stcoeffst,'String');
w1stcoeffind=get(w1stcoeffst,'Value');
w1stcoeff=eval(w1stcoeffarr{w1stcoeffind});

w2ndcoeffst=findobj('Tag','w2ndcoeff');
w2ndcoeffarr=get(w2ndcoeffst,'String');
w2ndcoeffind=get(w2ndcoeffst,'Value');
w2ndcoeff=eval(w2ndcoeffarr{w2ndcoeffind});

w3rdcoeffst=findobj('Tag','w3rdcoeff');
w3rdcoeffarr=get(w3rdcoeffst,'String');
w3rdcoeffind=get(w3rdcoeffst,'Value');
w3rdcoeff=eval(w3rdcoeffarr{w3rdcoeffind});

w1vec=(w1beg:wspacing:w1end);

scanparams.w1vec=w1vec;
scanparams.w2nd=w2nd;
scanparams.w3rd=w3;
scanparams.w1c=w1stcoeff;
scanparams.w2c=w2ndcoeff;
scanparams.w3c=w3rdcoeff;
scanparams.tvec=tvec;

%scanparams=[w1vec,w2nd,w3,w1stcoeff,w2ndcoeff,w3rdcoeff,tvec];
errorflag=SDCcal(Params,Subparams,scanparams,Afwphase,Abwphase,Fileparams);

if (saveparams==1)
    if (errorflag==0)
    %wvec=[w1beg;w1end;w2beg;w2end;w2sp];
    save(saveparamfile,'Params','Subparams','Fileparams2','scanparams');
