function output=resp2L(params,Eflags,t,begw,endw)
%
% DOVE-IR path 2 subroutine using Mukamel equations
% 
% ----------------------------------------------------------
% This routine performs a homogeneously broadened DOVE-IR path 2 2d-integral using
...
%				(only Gbg, and Gcb are used in this subroutine)
%
...
%  Eflags = a vector with indices of:
%   [0]:   E1 is Gaussian if 0, Abs(Exponential) in time (Lorentz, w) if 1
%   [1]:   If E1 is exponential:  both terms = 0, rising in time=1, falling
%   in time = 2
%   [2]:   E2, see[0]
%   [3]:   E2, see[1]
%   pulse shapes for E3 are not included, it is assumed E3 is always
%   gaussian.
%
% begw=beginning index value corresponding to the minimum w1 value of interest
...
n=length(t1vec);
radix=ceil(log2(M));
N=2^(radix); % slightly speeds up FFT algorithm
Tfin=N*del;
...
clear('E3arg');
clear('E3');
pdbtrans=fftshift(pdb)*(i*del/hbar)*(i*Tfin/hbar); % see NOTE
output=pdbtrans;
